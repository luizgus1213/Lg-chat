...
export function GroupInfoDialog({
  ...
  function saveDetails(event: FormEvent<HTMLFormElement>) {
    ...
    void run("details", (signal) =>
      updateGroup(
        chatId,
        { name: cleanName, description: cleanDescription || null },
        { signal },
      ),
    );
  }
  ...
  function changeAvatar(file: File | null) {
    ...
    if (file.size <= 0 || file.size > 2 * 1024 * 1024) {
      setError("A foto do grupo deve ter no máximo 2 MB.");
      return;
    }
  ...
  return (
    <Modal
      ...
      description={
        group ? `${members.length} membro(s)` : "Carregando detalhes do grupo."
      }
      onClose={() => {
      ...
      {status === "ready" && group ? (
        confirmation ? (
          <div
            ...
            role="alertdialog"
            aria-label="Confirmar ação do grupo"
          >
            <strong>
              {confirmation === "leave"
                ? "Sair deste grupo?"
                : confirmation === "delete"
                  ? "Excluir este grupo para todos?"
                  : `Remover ${confirmation.remove.user.nome}?`}
            </strong>
            <span>
              {confirmation === "delete"
                ? "O histórico e o grupo serão removidos permanentemente."
              ...
              <button
                ...
                onClick={() => {
                  if (confirmation === "leave")
                    void run(
                      "leave",
                      (signal) => leaveGroup(chatId, { signal }),
                      "remove",
                    );
                  else if (confirmation === "delete")
                    void run(
                      "delete",
                      (signal) => deleteGroup(chatId, { signal }),
                      "remove",
                    );
                  else
                    void run("remove", (signal) =>
                      removeGroupMember(chatId, confirmation.remove.userId, {
                        signal,
                      }),
                    );
                }}
          ...
          <div className={styles.layout}>
              ...
              {canManage ? (
                <>
                  <label className={styles.fileButton} htmlFor={avatarId}>
                    {busyAction === "avatar" ? "Enviando…" : "Alterar foto"}
                  </label>
                  <input
                    id={avatarId}
                    ...
                    disabled={isBusy}
                    onChange={(event) =>
                      changeAvatar(event.target.files?.[0] ?? null)
                    }
                  />
                </>
              ) : null}
            ...
            <section className={styles.members} aria-labelledby="members-title">
              <div className={styles.sectionTitle}>
                <strong id="members-title">Membros</strong>
              ...
              {canManage && availableUsers.length > 0 ? (
                <div className={styles.addMember}>
                  ...
                  <button
                    ...
                    onClick={() =>
                      void run("add", async (signal) => {
                        await addGroupMember(chatId, Number(selectedUserId), {
                          signal,
                        });
                        setSelectedUserId("");
                      })
                  ...
                  >
                    {busyAction === "add" ? "Adicionando…" : "Adicionar"}
                  </button>
                </div>
              ) : null}
                  ...
                  <li key={member.id}>
                    ...
                    {canManage && member.userId !== currentUserId ? (
                      <button
                        type="button"
                        disabled={isBusy}
                        onClick={() => setConfirmation({ remove: member })}
                      >
                        Remover
                      </button>
                    ) : null}
